...
function FieldLabel({ children, required }: { children: React.ReactNode; required?: boolean }) {
  return (
    <label className="block font-[Cinzel,'Palatino_Linotype',serif] text-[0.6rem] tracking-[0.22em] uppercase text-[#8a6d3b] mb-2">
      {children}
...
function SectionHeader({ num, children, sub }: { num: string; children: React.ReactNode; sub?: boolean }) {
  return (
    <div className="flex items-center gap-[0.7rem] mb-5 pb-2 border-b border-[#ede5d4] font-[Cormorant_Garamond,'Georgia',serif] text-[1.38rem] font-light text-[#13110e]">
      <span
        className={`font-[Cinzel,'Palatino_Linotype',serif] text-[0.65rem] text-[#c9a96e] border border-[#e8d5a3] w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 ${
          sub ? 'bg-[#f0e8d5]' : 'bg-[#fdf8ef]'
        }`}
      >
        {num}
      </span>
...
const inputCls =
  'w-full px-4 py-[0.82rem] border border-[#ddd5c0] rounded-[3px] font-[Lato,sans-serif] text-[0.9rem] font-light text-[#2c2416] bg-[#fafaf8] transition-all focus:outline-none focus:border-[#c9a96e] focus:ring-[3px] focus:ring-[rgba(201,169,110,0.1)]';
...
function ChoiceRow({
  ...
  return (
    <label
      onClick={onClick}
      className={`flex items-center gap-3 cursor-pointer text-[0.88rem] text-[#2c2416] px-[0.95rem] py-[0.6rem] border rounded-[3px] transition-all leading-snug ${
        selected
          ? 'border-[#c9a96e] bg-[#fdf8ef]'
          : 'border-[#ddd5c0] bg-[#fafaf8] hover:border-[#c9a96e] hover:bg-[#fdf8ef]'
      }`}
    >
      <input
        type={type}
        ...
        checked={selected}
        onChange={() => {}}
        className="accent-[#c9a96e] w-[14px] h-[14px] flex-shrink-0"
      />
      {children}
    </label>
  );
...
export default function RSVPForm() {
  ...
  if (submitted) {
    return (
      <div className="max-w-[700px] mx-auto px-8 py-20 bg-white">
        <div className="text-center p-[2.8rem_1.5rem] bg-gradient-to-br from-[#fdf8ef] to-[#f8ecc0] border border-[#e8d5a3] rounded-[6px] mt-4">
          <div className="text-[2.2rem] mb-3">✓</div>
          <h3 className="font-[Cormorant_Garamond,'Georgia',serif] text-[1.85rem] font-light text-[#13110e] mb-2">
            Thank You!
          </h3>
          <p className="text-[#7a6a52] text-[0.9rem] leading-[1.75]">
            Your RSVP has been received. A confirmation email will be sent to{' '}
            <strong>{formData.email}</strong>. We look forward to celebrating with you on 14 June 2026 in Alexandria!
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="max-w-[700px] mx-auto px-8 py-20 bg-white">
      {/* Page heading */}
      <p className="font-[Cinzel,'Palatino_Linotype',serif] text-[0.6rem] tracking-[0.42em] uppercase text-[#8a6d3b] mb-3 flex items-center gap-3 after:content-[''] after:flex-1 after:max-w-[48px] after:h-px after:bg-[#8a6d3b] after:opacity-45">
        RSVP &amp; Logistics
      </p>
      <h2 className="font-[Cormorant_Garamond,'Georgia',serif] text-[clamp(1.8rem,4.5vw,2.7rem)] font-light leading-[1.2] text-[#13110e] mb-5">
        Confirm Your <em className="italic text-[#8a6d3b]">Attendance</em>
      </h2>

      {/* Deadline box */}
      <div className="flex items-center gap-4 bg-gradient-to-br from-[#fdf6e3] to-[#f8ecc0] border-l-[3px] border-[#c9a96e]  px-[1.4rem] py-[0.95rem] mb-9 text-[0.86rem] text-[#7a6a52] rounded-r-[4px] leading-[1.7]">
        <CalendarDays />
        <div className=" ">
      ...
      <form onSubmit={handleSubmit} noValidate>
        ...
        <hr className="border-none border-t border-[#ede5d4] my-7" />
        ...
        {/* ── STEP 4: Events ── */}
        <div>
          <SectionHeader num="4">Which Events Will You Attend?</SectionHeader>
              ...
              <label
                ...
                className={`flex items-start gap-[0.65rem] border rounded-[3px] p-[0.8rem] cursor-pointer transition-all ${
                  formData.events.includes(ev.value)
                    ? "border-[#c9a96e] bg-[#fdf8ef] shadow-[0_0_0_1px_#c9a96e]"
                    : "border-[#ddd5c0] bg-[#fafaf8] hover:border-[#c9a96e] hover:bg-[#fdf8ef]"
                }`}
              >
                <input
                  ...
                  onChange={() => {}}
                  className="accent-[#c9a96e] mt-[3px] flex-shrink-0"
                />
                <div>
                  <div className="font-normal text-[0.85rem] text-[#13110e]">
                    {ev.label}
                  </div>
                  <div className="text-[0.72rem] text-[#7a6a52] mt-[2px]">
                    {ev.detail}
                  </div>
                </div>
              </label>
        ...
        {/* ── STEP 5: +1 ── */}
        <div>
          <SectionHeader num="5">Are You Bringing a Guest? (+1)</SectionHeader>
          <div className="flex flex-col gap-2 mb-6">
            {[
        ...
        {formData.has_plusone === "yes" && (
          <>
            ...
            <div>
              ...
              <div className="bg-[#fdf8ef] border border-[#e8d5a3] rounded-[4px] p-5">
              ...
              <p className="text-[0.78rem] text-[#b8a888] italic mt-2">
                Please fill in your guest's name and relationship to continue.
              </p>
            </div>
          </>
        )}
        ...
        <div>
          ...
          <div className="flex flex-col gap-2 mb-6">
            {[
              {
                value: "sunrise",
                label: "Yes — I'd like to stay at Sunrise Alex Avenue Hotel",
              },
              {
                value: "other",
        ...
        <div>
          ...
          <p className="text-[0.88rem] text-[#2c2416] mb-3">
        ...
        <div>
          ...
          <div className="mb-6">
            <FieldLabel>
              A message for Dioscore &amp; Specy (optional)
            </FieldLabel>
            <textarea
              className={inputCls + " resize-y min-h-[80px]"}
              name="message"
              placeholder="Share your well-wishes, memories, or any final thoughts..."
          ...
          <button
            type="submit"
            disabled={submitting}
            className="w-full py-4 px-8 bg-gradient-to-br from-[#7a5e28] to-[#c9a96e] text-[#13110e] border-none rounded-[3px] font-[Cinzel,'Palatino_Linotype',serif] text-[0.75rem] tracking-[0.28em] uppercase cursor-pointer font-semibold mt-2 transition-all hover:opacity-[0.88] hover:-translate-y-px disabled:opacity-50 disabled:cursor-not-allowed disabled:translate-y-0"
          >
            {submitting ? "Sending..." : "Submit Your RSVP"}
          </button>
          <p className="text-center text-[0.76rem] text-[#7a6a52] mt-[0.9rem] italic leading-[1.6]">
